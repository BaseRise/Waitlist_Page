      ...
      {status === 'success' ? (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 py-8">
          ...
          <div className="max-w-md mx-auto bg-white/5 border border-white/10 p-2 rounded-2xl flex items-center gap-2">
            ...
            <button onClick={copyToClipboard} className="bg-blue-600 hover:bg-blue-700 cursor-pointer p-2.5 rounded-xl transition-all flex items-center gap-2 text-xs font-bold min-w-[90px] justify-center text-white">
              {copied ? <Check size={14} /> : <Copy size={14} />}
              {copied ? 'COPIED' : 'COPY'}
            </button>
          </div>
          <button onClick={() => setStatus('idle')} className="text-[10px] text-gray-500 hover:text-white transition-colors uppercase tracking-[0.2em] font-bold">Back to waitlist</button>
        </motion.div>
      ) : (
        <div className="animate-in fade-in duration-700 w-full">
          ...
          <div className="max-w-md mx-auto px-4">
            <form onSubmit={handleSubmit} className="input-container p-1.5 rounded-2xl flex flex-col md:flex-row items-center gap-2 md:gap-0 bg-white/5 border border-white/10">
              ...
              <button type="submit" disabled={status === 'loading'} className="btn-shimmer cursor-pointer disabled:cursor-not-allowed w-full md:w-auto px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest text-white disabled:opacity-50">
            ...
            <div className="mt-8 flex flex-col items-center gap-6">
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <div className="flex items-center justify-center gap-4">
                    <a href="#" className="group relative w-12 h-12 flex items-center justify-center rounded-2xl bg-[#121212] border border-white/5 transition-all duration-200 ease-out hover:scale-110 hover:drop-shadow-lg">
                    ...
                    <a href="#" className="group relative w-12 h-12 flex items-center justify-center rounded-2xl bg-[#121212] border border-white/5 transition-all duration-200 ease-out hover:bg-[#5865F2]/20 hover:scale-110 hover:drop-shadow-lg">
